"""Backend sobre os dados reais da fila de creche do Rio — `MapaFilaCreche/`.
...
`chance = confirmados ÷ demanda_de_1ª_opção`, na própria unidade, em 2025. É a fração
observada de quem pediu aquela creche como primeira opção e acabou atendido — a razão
candidato/vaga que os dados sustentam.
...
fevereiro, e fila cheia no ano passado não fecha a porta deste ano.
"""
...
# Começa no raio que as famílias aceitam de fato — 82,9% dos que trocaram de creche
...
class _Unidade:
    ...
    ociosas: int         # vagas livres na unidade, somando os grupamentos
...
def _unidades() -> tuple[_Unidade, ...]:
    ...
    for linha in _linhas("mapa_unidades.csv"):
        ...
        unidades.append(_Unidade(
            ...
            ociosas=sum(ociosas.get(desig7, {}).values())))
...
def chance_em(unidade: _Unidade, tem_vaga_ociosa: bool) -> float | None:
    """A fração de quem pediu esta creche como 1ª opção em 2025 e foi atendido.
...
class BackendMapa(BackendMock):
    ...
    def panorama_da_regiao(self, endereco: Endereco) -> PanoramaRegiao | None:
        ...
        return PanoramaRegiao(
            ...
            demanda=int(_num(linha["demanda"])), atendidos=int(_num(linha["conf"])),
