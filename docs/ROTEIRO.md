...
`/ConsultaCreche` do portal, porque nem todo mundo guarda o número, e há criança sem
filiação na certidão. A regra crítica é uma só: o que aparece é o `Desfecho`, nunca o
status bruto por opção ([D14](DECISOES.md)).
...
Duas perguntas da régua o bot nunca faz, porque já sabe a resposta: "esperou na fila no ano
anterior" (vem de `CadastroAnterior`, e sai validada) e "responsável menor de 18" (da data de
...
| `/avancar` | Empurra uma etapa para demonstrar a notificação. Existe enquanto o backend for o mock, e o `BackendMapa` herda dele, então vale nos dois |
...
| Eco de toda resposta, nunca da sensível | `formulario_passo._eco`, e `Campo.sensivel` é a única exceção: o histórico fica no aparelho da família |
...
Sobre uma creche vão distância, vaga ociosa agora no grupamento pedido, concorrência do ano
