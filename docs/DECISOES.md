...
`BackendMock` por baixo, e o `BackendHTTP` entra quando o outro time publicar. Nada do JSON
...
**Por que caiu a nota de corte:** ela é a pontuação do último aprovado, e o teto da régua foi 465
...
2. As cinco perguntas num turno só, como checklist, que é a exceção deliberada à regra de uma
...
> Revisto, ver [D21](#d21--postgres-no-supabase-em-schema-próprio-sem-orm). O SQLite saiu e
> entrou o Postgres, então existe uma dependência de runtime, o `psycopg`.
...
1. O ano vai colado no número, em toda tela, e há teste.
...
2. Falha vira frase, e não código HTTP. `MOTIVOS` traduz 401, 403, 404, 429, 5xx, rede fora
...
3. Falha no meio da conversa também é dita, uma vez por queda, e não a cada mensagem. O
...
tabelas ficam no schema `creche`, e não no `public`. O `sqlite.py` da validação foi
removido.
...
`documento_crianca` passou a receber o CPF que o bloco 1 pede, que antes era coletado e
...
para alcançar banco que já existe, que é o preço de não ter Alembic
([D21](#d21--postgres-no-supabase-em-schema-próprio-sem-orm)).
